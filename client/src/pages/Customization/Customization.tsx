...
import {
  ...
  B_I2,
  T_I4,
  BASE_UPLOADS_MANNEQUINS_BACKS_URL,
...
const Customization = () => {
  ...
  const dataProduct = useSelector(getDataProductInfo);
    ...
    front: [
      {
        src: B_I2,
        color: activeColor,
        printImageURL: activePrint?.highresurl,
        activeCategory,
      },
      {
        src: T_I4,
  ...
  const canvasModelInit = (
    num: number,
    modData: ArrayType,
    frontBack: string = "front"
  ) => {
    ...
    img.src =
      frontBack === "front"
        ? `${BASE_UPLOADS_MANNEQUINS_FRONTS_URL}${mannequins?.[0]?.fronturl}`
        : `${BASE_UPLOADS_MANNEQUINS_BACKS_URL}${mannequins?.[0]?.backurl}`;
    // img.src = M_I;
    img.onload = async () => {
      await ctx?.drawImage(img, 0, 0, width, height);
      for (let i = 0; i < modData.length; i++) {
        console.log( modData[i]," modData[i]");
    ...
    if (mannequins?.length) {
      const drawManequin = async () => {
        for (let i = 0; i < modelData[frontBack].length; i++) {
          console.log(modelData[frontBack], "modelData");

          modelData.front[i].activeCategory = activeCategory;
          switch (activeType) {
            case "tops":
			 modelData.front[1].src = activeImgUrl;
              setModelData({...modelData});
              // dispatch(setDataInfo({...dataProduct,silhouette:{...silhouette,front:{top:modelData[1].src}}}))
              break;
            case "bottoms":
			modelData.front[0].src = activeImgUrl;
              ...
              break;
            case "sleeves":
			modelData.front[2].src = activeImgUrl;
              setModelData(modelData);
              break;
            case "top":
              modelData.front[1].color = activeColor;
              setModelData(modelData);
              if (activePrint) {
                modelData.front[1].printImageURL = activePrint?.highresurl;
              }
              break;
            case "bottom":
              modelData.front[0].color = activeColor;
              setModelData(modelData);
              if (activePrint) {
                modelData.front[0].printImageURL = activePrint?.highresurl;
              }
              break;
            default:
              modelData.front[i].color = activeColor;
              modelData.front[i].printImageURL = activePrint?.highresurl;
              setModelData(modelData);
          }
        }
        await canvasModelInit(0.2, modelData, frontBack);
      };
      drawManequin();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
